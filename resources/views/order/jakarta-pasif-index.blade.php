...
@section('title', 'Jakarta Pasif - biMBA AIUEO Logistik')
...
<style>
    ...
    .nav-dropdown:hover { border-color: #6366f1; }
    .nav-dropdown:focus {
        outline: none;
        border-color: #4f46e5;
    ...
    .nav-back { background: #64748b; color: white; }
    .nav-back:hover { background: #475569; color: white; transform: translateY(-1px); }

    .nav-modul { background: #16a34a; color: white; }
    .nav-modul:hover { background: #15803d; color: white; }

    .nav-majalah { background: #2563eb; color: white; }
    .nav-majalah:hover { background: #1d4ed8; color: white; }

    .nav-sertifikat { background: #dc2626; color: white; }
    .nav-sertifikat:hover { background: #b91c1c; }

    .nav-semua { background: #334155; color: white; }
    .nav-semua:hover { background: #1e293b; color: white; }
    ...
    .action-title { display: flex; align-items: center; gap: 10px; }
    .action-icon {
        width: 36px;
        height: 36px;
        display: flex;
    ...
    .action-heading { color: #1e293b; font-size: 14px; font-weight: 700; }
    .action-subtitle { color: #64748b; font-size: 12px; margin-top: 2px; }
    ...
    @media (max-width: 768px) {
        .nav-tab, .nav-back, .btn-rekap { flex: 1 1 auto; }
        .action-bar { align-items: stretch; }
        .action-title { width: 100%; }
...
<div class="max-w-screen-2xl mx-auto px-6 py-6">
    ...
    @if(session('unit_nama_mismatch') && count(session('unit_nama_mismatch')) > 0)
        <div class="mb-6 bg-orange-50 border border-orange-300 text-orange-900 px-5 py-4 rounded-2xl">
            <div class="flex items-start gap-3">
                <svg class="w-5 h-5 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 9v4m0 4h.01M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></svg>
                <div class="flex-1">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Jakarta Pasif</h1>
            <p class="text-gray-600">Kelola Data Order Jakarta Pasif (JKTP)</p>
        </div>
                ...
                <div class="action-bar">
                    <div class="action-buttons">
                        ...
                        <div class="flex items-center gap-3 flex-wrap">
                            <form action="{{ route('order.jakarta-pasif.sync-jkt') }}"
                                    method="POST"
                                    onsubmit="return confirm('Yakin ingin melakukan Sync JKTP + Casdana?')">
                                @csrf
                                <button type="submit" class="btn-action btn-sync">
                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 2v6h-6"/><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M3 22v-6h6"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/></svg>
                                    <span>Sync JKTP + Casdana</span>
                                </button>
                            </form>
                        </div>

                        {{-- KANAN: REKAP --}}
                        <a href="{{ route('order.jakarta-pasif-printed') }}" class="btn-rekap">
                        ...
                        {{-- EXPORT --}}
                        <a href="{{ route('order.jakarta-pasif.export') }}{{ $queryString ? '?' . $queryString : '' }}" class="btn-action btn-export">
    ...
    <div class="bg-slate-100 rounded-3xl shadow p-6 mb-6">
        <form method="GET" id="filterForm" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-9 gap-4">
            ...
            <div class="flex items-end gap-3 pt-6 lg:col-span-1 xl:col-span-1">
                <button type="submit"
                        class="bg-blue-600 text-white px-6 py-2.5 rounded-xl hover:bg-blue-700 flex-1 inline-flex items-center justify-center gap-2">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="{{ $isProcessed ? 'processed-row' : '' }} hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            @if($isMismatch)
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-orange-100 text-orange-800 border border-orange-200"
                                    title="Nama unit beda dengan Unit Kemitraan">
                                    <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 9v4m0 4h.01M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></svg>
                                    Mismatch
                                </span>
                            @endif
                    ...
                    <td class="text-center px-4 py-3">
                        @if(!$isProcessed)
                            <a href="{{ route('order.jakarta-pasif.edit', $item->id) }}"
                            class="text-blue-600 hover:text-blue-700 inline-block hover:scale-110 transition">
                                <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 1 1 2.828 2.828l-.793.793-2.828-2.828.793-.793ZM11.379 5.793 3 14.172V17h2.828l8.38-8.379-2.83-2.828Z"/></svg>
                            </a>
                        @else
                            <span class="inline-flex items-center gap-1 text-emerald-600 font-medium text-sm">
                                <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                Diproses
                            </span>
                        @endif
                    </td>
                ...
                <tr>
                    <td colspan="24" class="text-center py-16 text-gray-500">
                        Belum ada data Jakarta Pasif.<br>
                        Silakan klik tombol <strong>Sync JKTP + Casdana</strong>.
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div id="bulkModal" class="hidden fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div class="bg-white rounded-3xl shadow-2xl w-[98vw] h-[95vh] mx-2 flex flex-col">
            <div class="p-6 border-b flex justify-between items-center">
                ...
                <button onclick="hideBulkModal()" class="text-gray-500 hover:text-gray-700">
                    <svg class="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <div class="p-6 border-t bg-gray-50 flex justify-end gap-3">
                ...
                <button onclick="executeBulkAction()" class="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-semibold hover:bg-indigo-700 inline-flex items-center gap-2">
                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2Z"/><path d="M17 21v-8H7v8M7 3v5h8"/></svg>
...
@push('scripts')
{{-- Kalau layouts.panel sudah memuat jQuery, hapus baris berikut agar tidak dobel --}}
...
<script>
    ...
    function processAllFilteredData() {
        ...
        $.ajax({
            url: '{{ route("order.jakarta-pasif.filtered-ids") }}',
            method: 'GET',
            data: {
                route: currentRoute,
                ...
                status_pembayaran: $('#filterStatusBayar').val() || ''
            },
    ...
    function loadModalData() {
        $.ajax({
            url: '{{ route("order.jakarta-pasif.get-modal-data") }}',
            method: 'POST',
            data: {
                ids: selectedIds,
                _token: '{{ csrf_token() }}'
        ...
        function checkSaveButtonState() {
            ...
            if (isValid) {
                saveButton.prop('disabled', false)
                          ...
                          .html('<svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2Z"/><path d="M17 21v-8H7v8M7 3v5h8"/></svg> Simpan & Kunci Semua Data');
            } else {
    ...
    function executeBulkAction() {
        ...
        const form = $('<form>', {
            action: '{{ route("order.jakarta-pasif.bulk-action") }}',
            method: 'POST'
        });
        ...
        $('<input>', { type: 'hidden', name: 'redirect', value: currentRoute }).appendTo(form);
