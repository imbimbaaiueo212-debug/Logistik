...
@section('title', 'Detail Pesanan Majalah PUW1')
...
@section('content')
    ...
    @php
        ...
        // Contact person: nama dan telepon dipasangkan berurutan -> "Nama (0812...), Nama2 (0813...)"
        $names  = array_values(array_filter(array_map('trim', explode(',', $data->contact_person ?? ''))));
        $phones = array_values(array_filter(array_map('trim', explode(',', $data->telepon_contact_person ?? ''))));
        $combined = [];
        foreach ($names as $i => $name) {
            $phone = $phones[$i] ?? null;
            $combined[] = $phone ? "{$name} ({$phone})" : $name;
        }
        $contactText = !empty($combined) ? implode(', ', $combined) : '-';

        // Info periode: [label, isi, melebar 2 kolom?]
        $info = [
            ['Judul', $data->judul ?? '-'],
            ['Bulan / Edisi', $data->bulan ?? '-'],
            ['Tahun', $data->tahun ?? '-'],
            ['Periode', $periodeText],
            ['Contact Person', $contactText, true],
        ];
        ...
        $filters = [
            ...
            ['kabupaten_kota', 'Kabupaten / Kota', 'Semua kabupaten/kota', $listKabupaten],
        ];

        // Kolom tabel setelah "No": [judul, field, tipe]. Tipe: text, clip, num
        $kolom = [
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
            <h1 class="text-2xl sm:text-[28px] leading-tight font-bold text-navy-950">Detail Pesanan Majalah PUW1</h1>
            <p class="mt-1 text-sm text-navy-950/55">{{ $data->judul ?? 'Pesanan Majalah PUW1' }} — {{ $data->bulan ?? '-' }} {{ $data->tahun ?? '' }}</p>
        </div>
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-card p-5 sm:p-6">
        ...
        <div class="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-5">
            @foreach ($info as $it)
                @php $lebar = $it[2] ?? false; @endphp
                <div class="min-w-0 {{ $lebar ? 'col-span-2' : '' }}">
                    <p class="text-xs text-navy-950/50 mb-1">{{ $it[0] }}</p>
                    <p class="text-sm font-semibold text-navy-950 {{ $lebar ? 'break-words' : 'truncate' }}" @unless ($lebar) title="{{ $it[1] }}" @endunless>{{ $it[1] }}</p>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mt-4 bg-white rounded-2xl shadow-card p-5 sm:p-6">
        <form method="GET" action="{{ route('pesanan-majalah-puw1.show', $data->id) }}">
    ...
    <div class="mt-4 bg-white rounded-2xl shadow-card overflow-hidden">
        <div class="px-5 sm:px-6 py-4 border-b border-navy-950/10 flex flex-wrap items-center justify-between gap-2">
            <h2 class="text-base font-semibold text-navy-950">Daftar Unit PUW1</h2>
            <p class="text-sm text-navy-950/55">
                Total unit: <span class="font-semibold text-navy-950">{{ number_format($units->count(), 0, ',', '.') }}</span>
                <span class="mx-2 text-navy-950/25">|</span>
                Total pesanan: <span class="font-semibold text-navy-950">{{ number_format(round($totalPesanan), 0, ',', '.') }}</span>
            </p>
        </div>
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            @foreach ($kolom as $kol)
                                @php
                                    ...
                                    $val  = $unit->{$kol[1]};
                                    if ($tipe === 'num') {
                                        $val = number_format(round((float) ($val ?? 0)), 0, ',', '.');
                                    }
                                    $tampil = filled($val) ? $val : '-';
                                @endphp
                                <td class="{{ $kol[1] === 'nama_unit' ? 'font-medium' : '' }} {{ $tipe === 'num' ? 'ctr font-semibold' : '' }} {{ $tipe === 'clip' ? 'cell-clip' : '' }}"
                                    @if ($tipe === 'clip') title="{{ $tampil }}" @endif>{{ $tampil }}</td>
                            @endforeach
                        </tr>
                    @empty
                        <tr>
                    ...
                    @if ($units->count() > 0)
                        <tr class="baris-total">
                            ...
                            <td class="ctr">{{ number_format(round($totalPesanan), 0, ',', '.') }}</td>
                            <td colspan="{{ count($kolom) - 4 }}"></td>
                        </tr>
                    @endif
                </tbody>
            </table>
