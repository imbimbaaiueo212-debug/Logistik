...
@section('title', 'Data Manual Order')

@section('content')

    @php
        // Kartu menu: [judul, keterangan, url, ikon]. Ikon: gedung, edit
        $menu = [
            ['Unit Operasional 2 (OPS2)', 'KORWIL · PINWIL · JABODETABEK', route('ops2.index'), 'gedung'],
            ['DLC', 'Input data pemesanan majalah DLC.', route('import.dlc.index'), 'gedung'],
            ['Unit Pasif', 'Input data pemesanan majalah Pasif.', route('import.pasif.index'), 'gedung'],
            ['Manual Pemesanan', 'Input data pemesanan secara manual, termasuk produk, jumlah, dan harga.', route('import.manual'), 'edit'],
        ];
    @endphp

    {{-- ============ JUDUL ============ --}}
    <div class="min-w-0">
        <h1 class="text-2xl sm:text-[28px] leading-tight font-bold text-navy-950">Data Manual Order</h1>
        <p class="mt-1 text-sm text-navy-950/55">Pilih jenis pemesanan yang akan diinput</p>
    </div>
