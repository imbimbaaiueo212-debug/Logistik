...
<head>
    ...
    <title>Edit Manual Modul - biMBA AIUEO Logistik</title>
...
<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            <h1 class="text-lg font-semibold text-gray-800">Edit Manual Modul</h1>
            <p class="text-sm text-gray-500">
                ID Manual: <strong>MM-{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</strong>
                @if($order->order_id)
    ...
    <form id="formManualModul" action="{{ route('order-manual-modul.manual.update', $order->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-xs text-gray-500 border-b">
                            ...
                            <th class="pb-2 pr-2 font-medium" style="min-width:180px">Nama Produk <span class="text-red-500">*</span></th>
...
if (selectedVal) {
    $('#customerSelect').val(selectedVal).trigger('change');
}
...
$(document).ready(function () {
    ...
    const address2     = opt.data('address-2') || '';   // ← ini yang hilang
    ...
    $('#billing_address_2').val(address2);             // ← set Address line 2
    $('#billing_city').val(city);
    $('#phone').val(phone);

    // Shipping ikut terisi
    $('#shipping_address_1').val(address1);
    $('#shipping_address_2').val(address2);            // ← set juga di Shipping
