...
@section('title', 'Pesanan Majalah Kotamadya')
...
@section('content')
    ...
    @php
        ...
        $kolom = ['No', 'Judul Pesanan', 'ID_PS', 'Bulan / Edisi', 'Tahun', 'Periode', 'Jumlah Unit', 'Total Pesanan'];
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
            <h1 class="text-2xl sm:text-[28px] leading-tight font-bold text-navy-950">Pesanan Majalah Kotamadya</h1>
            <p class="mt-1 text-sm text-navy-950/55">Daftar periode pesanan majalah Kotamadya. Klik judul untuk melihat detail data Kotamadya dan unit.</p>
        </div>
                    ...
                    @forelse ($data as $item)
                        ...
                        <tr>
                            ...
                            <td class="cell-clip" title="{{ $judulText }}">
                                <a href="{{ route('pesanan-majalah-kotamadya.show', $item->id) }}"
                                   class="font-semibold text-navy-700 hover:text-rust-600 hover:underline">{{ $judulText }}</a>
                            </td>

                            {{-- ID_PS (klik untuk mengubah, tersimpan otomatis) --}}
                            <td class="ctr">
                                <div class="no-ps-wrapper inline-flex items-center justify-center"
                                     data-url="{{ url('pesanan-majalah-kotamadya/' . $item->id . '/update-no-ps') }}">
                            ...
                            <td class="ctr font-medium">{{ number_format($item->total_units ?? 0, 0, ',', '.') }}</td>
                            <td class="ctr font-semibold text-emerald-700">{{ number_format($item->total_pesanan ?? 0, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ count($kolom) }}" class="!text-center py-16 text-navy-950/50">
                                <p class="font-semibold text-navy-950/70">Belum ada data</p>
                                <p class="mt-1">Belum ada periode pesanan majalah Kotamadya.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div id="importModal" class="hidden fixed inset-0 z-50 bg-navy-950/50 items-center justify-center px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div class="flex items-start justify-between px-6 py-5 border-b border-navy-950/10">
                <div>
                    <h2 class="text-lg font-bold text-navy-950">Import Pesanan Majalah Kotamadya</h2>
                    <p class="text-sm text-navy-950/55 mt-0.5">Pilih periode tujuan import data.</p>
                </div>
            ...
            <form id="formImportMajalahKotamadya" action="{{ route('pesanan-majalah-kotamadya.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                ...
                <div>
                    ...
                    <select id="periodeImport" name="periode" required class="{{ $inp }}">
                        ...
                        @php $tanggalMulai = now()->startOfMonth(); @endphp
                        @for ($i = 0; $i <= 12; $i++)
                            @php $tgl = $tanggalMulai->copy()->addMonths($i); @endphp
                            <option value="{{ $tgl->format('Y-m') }}">{{ $tgl->translatedFormat('F Y') }}</option>
                        @endfor
                    </select>
                    <p class="mt-1.5 text-xs text-navy-950/45">Rentang 13 bulan, dimulai dari bulan ini.</p>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var form      = document.getElementById('formImportMajalahKotamadya');
    ...
    document.addEventListener('focusout', function (e) {
        ...
        .then(function (data) {
            if (!data.success) throw new Error(data.message || 'Gagal menyimpan ID_PS');
        ...
        .catch(function (err) {
            ...
            alert(err.message || 'Terjadi kesalahan saat menyimpan ID_PS');
        });
    });
});
</script>
