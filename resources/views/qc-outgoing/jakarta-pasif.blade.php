...
@section('title', 'QC Outgoing - Jakarta Pasif - biMBA Logistik')
...
<div class="max-w-screen-2xl mx-auto px-6 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">QC Outgoing - Jakarta Pasif</h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        {{ $item->tgl_turun_pl ? \Carbon\Carbon::parse($item->tgl_turun_pl)->format('d/m/Y') : '-' }}
                    </td>
                    ...
                    <form method="POST" action="{{ route('qc-outgoing.pasif.store') }}">
                        @csrf

                        <input type="hidden" name="id" value="{{ $item->id }}">
                        <input type="hidden" name="picking_pasif_id" value="{{ $item->picking_pasif_id }}">
                        ...
                        <td class="px-4 py-3">
                            @if($item->pic_qc)
                                ...
                                <input type="hidden" name="pic_qc" value="{{ $item->pic_qc }}">
                            @else
                                <select name="pic_qc"
                                        class="border border-gray-300 rounded-lg px-3 py-1 text-sm w-full"
                                        required>
                ...
                @empty
                <tr>
                    <td colspan="14" class="text-center py-10 text-gray-500">Tidak ada data</td>
                </tr>
                @endforelse
            </tbody>
        </table>
