    ...
    <div class="bg-white rounded-3xl shadow p-6 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <select name="service_pengiriman" class="w-full border border-gray-300 rounded-2xl px-4 py-3 text-sm">
                    <option value="">Semua</option>
                    @foreach($serviceList ?? [] as $svc)
                        <option value="{{ $svc }}" {{ request('service_pengiriman') == $svc ? 'selected' : '' }}>{{ $svc }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($distributionOrders as $item)
                    <tr class="transition duration-200 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-4">
                            {{ optional($item->jakartaAktif)->ekspedisi ?? '-' }}
                        </td>
                        <td class="px-4 py-4">
                        ...
                        <td class="px-3 py-4">
                            <input type="text" name="awb" value="{{ $item->awb }}"
