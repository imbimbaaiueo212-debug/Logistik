...
@section('title', 'Detail Pesanan Majalah')
...
@section('content')
    ...
    @php
        ...
        $filters = [
            ...
            ['kabupaten', 'Kabupaten', 'Semua kabupaten', $listKabupaten],
            ['mitra_pengelolaan', 'Mitra Pengelola', 'Semua mitra', $listMitraPengelola],
        ];

        // Kolom tabel: [judul, field, tipe]. Tipe: text, clip, num
        $kolom = [
            ['Nama Unit', 'nama_unit', 'clip'],
            ['No Cabang', 'no_cabang'],
            ['Mitra Pengelola', 'mitra_pengelolaan', 'clip'],
            ['Jumlah Pesanan', 'jumlah_pesanan', 'num'],
            ['Alamat Unit', 'alamat_unit', 'clip'],
            ['Telepon', 'telepon'],
            ['Kabupaten', 'nama_kabupaten'],
            ['Contact Person', 'contact_person', 'clip'],
        ];
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
            <h1 class="text-2xl sm:text-[28px] leading-tight font-bold text-navy-950">Detail Pesanan Majalah</h1>
            <p class="mt-1 text-sm text-navy-950/55">{{ $data->judul ?? 'Pesanan Majalah' }} — {{ $data->bulan ?? '-' }} {{ $data->tahun ?? '' }}</p>
        </div>
    ...
    <div class="mt-4 bg-white rounded-2xl shadow-card p-5 sm:p-6">
        <form method="GET" action="{{ route('pesanan-majalah.show', $data->id) }}">
                    ...
                    @forelse ($units as $unit)
                        <tr>
                            ...
                            @foreach ($kolom as $i => $kol)
                                ...
                                @endphp
                                <td class="{{ $i === 0 ? 'font-medium' : '' }} {{ $tipe === 'num' ? 'ctr font-semibold' : '' }} {{ $tipe === 'clip' ? 'cell-clip' : '' }}"
